...
void GEMCluster::Configure([[maybe_unused]]const std::string &path)
{
    ...
    CONF_CONN(max_cluster_hits, "Max Cluster Hits", 200, verbose); //20
    CONF_CONN(split_cluster_diff, "Split Threshold", 14, verbose); //14
    CONF_CONN(cross_talk_width, "Cross Talk Width", 2, verbose);
    CONF_CONN(consecutive_thres, "Consecutive Threshold", 3, verbose); //1
